...
在resources中创建templates目录，在templates目录创建 index.html,代码如下：
...
`<p th:text="${message}"></p>`:这句使用 th:text="${变量名}" 表示 使用thymeleaf获取文本数据，类似于EL表达式。
...
```html
<a href="/test/index?age=${age}">去看首页</a>
<a th:href="@{/test/index(age=${age})}">去看首页</a>

<a href="/test/index?age=${age}&name=${age}">去看首页</a>
<a th:href="@{/test/index(age=${age},name=${age})}">去看首页</a>
```
 ...
 */
    ...
    public String showIndex(Model model){
        //key  vlaue
        model.addAttribute("message","zhangsanssss");

        //输出POJO
        Person person = new Person(1100L, "川建国");
        // request key value
        model.addAttribute("person",person);
        //输出集合
        List<Person> list = new ArrayList<>();
        list.add(new Person(1111L,"光头强"));
        list.add(new Person(1112L,"喜洋洋"));
        list.add(new Person(1113L,"金三胖"));
...
<table>
    ...
    <tr th:each="person:${mylist}">
        <td th:text="${person.id}">123</td>
        <td th:text="${person.name}">zhangsna</td>
...
![1585192076499](images/1585192076499.png)
...
![1585192028301](images/1585192028301.png)
...
需要修改search.html:将数据./css/xxx.js全部换成绝对路径
...
![1585193200033](images/1585193200033.png)
...
如果没有注解需要添加依赖 changgou-service-search-api 中添加
...
![1569219717125](images/1569219717125.png)
...
![1566736163765](images/1569219717125.png)
